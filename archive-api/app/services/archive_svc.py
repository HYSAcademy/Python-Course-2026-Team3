    ...
    async def process_archive_background(
        ...
        async with AsyncSessionLocal() as session:
            ...
            try:
                ...
                logger.info(
                    f"Archive ID={archive_id} successfully processed and saved to DB!"
                )
                ...
                if uploaded_to_s3:
                    try:
                        ...
                        logger.warning(
                            f"Failed to cleanup S3 object {s3_object_name}: {s3_err}"
                        )
                try:
                    ...
                    logger.critical(
                        f"Failed to update status in DB for {archive_id}: {db_err}"
                    )
